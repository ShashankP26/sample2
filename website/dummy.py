...
def get_used_and_remaining(group):
    used_total = (
    ...
    remaining = group.total_advance - used_total
    if remaining < 0:
        remaining = Decimal("0.00")

    return used_total, remaining
...
def assign_advances(request):
    ...
    if request.method == "POST":
        # quick guard - your older logic redirected if "new_amount" in POST
        ...
        # Create or get group
        group = AdvanceGroup.objects.filter(name__iexact=group_name).first()
        if not group:
            group = AdvanceGroup.objects.create(name=group_name, total_advance=Decimal("0.00"))

        previous_amount = group.total_advance
        group.total_advance = (group.total_advance or Decimal("0.00")) + amount
        group.save()

        # Log update (use uploaded proof if present)
        ...
        # Assign users and leader handling
        success, failed = 0, 0
        for uid in user_ids:
            try:
                user = User.objects.get(id=uid)
                is_leader = str(uid) == str(leader_id)
                ...
                logger.exception(f"Failed to assign user ID {uid}: {e}")
                failed += 1

        # Create allocation expense safely (clone file)
        proof_copy = None
        if proof_file:
            proof_file.seek(0)
        ...
        Expense.objects.create(
            ...
            internal_option=group.leader.get_full_name() if getattr(group, "leader", None) else "N/A",
            transaction_details=f"Group leader: {group.leader.get_full_name() if getattr(group, 'leader', None) else 'N/A'}",
            amount=group.total_advance,
            transaction_date=group.created_at.date() if group.created_at else datetime.now().date(),
            item_name=group.name,
            payment_mode="net_banking",
            proof_photo=proof_copy,
            advance_group=group,
            evoucher_number=generate_evoucher_number(),
            is_allocation=True,
        )
        ...
        messages.success(
            request,
            f"₹{amount} assigned to group '{group.name}'. {success} user(s) assigned. {failed} failed. Expense voucher created."
        )
        return redirect("assign_advances")
    ...
    if group_name:
        ...
        if selected_group:
            ...
            assigned_user_ids = list(
                AdvanceAssignment.objects.filter(group=selected_group).values_list("user_id", flat=True)
            )

            leader = AdvanceAssignment.objects.filter(group=selected_group, is_leader=True).first()
            leader_id = leader.user.id if leader else None

            # DO NOT save used/remaining on the model - compute live
            used_total, remaining_total, overdue_total = calculate_group_balances(selected_group)

            # keep logs for history view
            logs = list(AdvanceGroupUpdateLog.objects.filter(group=selected_group).order_by("timestamp"))
        else:
            logs = []
    else:
        logs = []
    ...
    context = {
        ...
        "assigned_user_ids_map": {
            g.id: list(AdvanceAssignment.objects.filter(group=g).values_list("user_id", flat=True))
            for g in AdvanceGroup.objects.all()
        },
        "groups_json": json.dumps({str(g.id): g.name for g in AdvanceGroup.objects.all()}),
    }
...
def calculate_group_balances(group):
    """
    Calculate used (sum of debits for non-allocation expenses) and remaining.
    Does NOT save anything to the DB — returns (used, remaining, overdue).
    """
    used = (
        Expense.objects.filter(
            item_type="advances",
            advance_group=group,
            is_allocation=False,   # ignore allocation entries
            is_draft=False,
    ...
    if remaining < 0:
        # remaining can't be negative for display; overdue will capture the negative part
        remaining = Decimal("0.00")

    overdue = used - group.total_advance if used > group.total_advance else Decimal("0.00")
...
def view_assigned_groups(request):
    ...
    try:
        ...
        start_date_dt = end_date_dt = None

    if start_date_dt and end_date_dt:
        groups_qs = groups_qs.filter(created_at__date__gte=start_date_dt, created_at__date__lte=end_date_dt)
    ...
    for group in groups_qs:
        assigned_user_ids = list(AdvanceAssignment.objects.filter(group=group).values_list("user_id", flat=True))
        ...
        if start_date_dt and end_date_dt:
            expenses = expenses.filter(date__range=[start_date_dt, end_date_dt])
        ...
        initial_proof = group.proof_file.url if getattr(group, "proof_file", None) else None
    ...
    all_users = User.objects.all()
    return render(request, 'xp/view_assigned_groups.html', {
...
def update_group_balance(request, group_id):
    ...
    prev = group.total_advance or Decimal("0.00")
    group.total_advance = prev + additional_amount
    group.save()
    ...
    messages.success(
        request,
        f"Added ₹{additional_amount} to group '{group.name}'. New total: ₹{group.total_advance}"
    )
    return redirect("assign_advances")
...
def edit_item(request, item_id):
    ...
    if request.method == "POST":
        ...
        # --- Draft handling ---
        draft_status = data.get("draft_status", str(is_draft)).lower()
        is_draft = draft_status == "true"
...
def payable(request):
    ...
    leader_sub_balance = Decimal('0.00')      # total positive remaining across leader's groups
    leader_overdue_total = Decimal('0.00')    # total overdue across groups
    leader_group_balances = []                # per-group details
    ...
    if request.method == 'POST':
        ...
        if from_date and to_date:
            try:
                ...
                queryset = queryset.filter(transaction_date__range=[f, t])
            except Exception:
                pass

        # Filter only vouchers that qualify
        expenses = queryset.annotate(proof_count=Count('proof_photos')).filter(
            ...
            Q(status='paid') | Q(status='rejected') | Q(is_rejected=True)
        ).filter(
            is_draft=False
        ).filter(
            Q(cash_vouchers__isnull=True) | Q(cash_vouchers__status='approved')
        )

        # total payable = sum of filtered expense amounts
        total_amount_to_be_paid = sum((exp.amount or Decimal('0.00')) for exp in expenses)

    # Always compute leader group balances even if no vouchers returned (per your request)
    if selected_user:
        ...
        # Final amount = payable - (sum of group remaining) + sum(overdue)
...
def pay_now(request, user_id):
    ...
    total_amount_to_be_paid = request.GET.get('total_amount_to_be_paid') or request.GET.get('amount') or 0
    total_amount_to_be_paid = Decimal(total_amount_to_be_paid)

    # Leader advance balances
    leader_assignments = AdvanceAssignment.objects.filter(user=selected_user, is_leader=True)
    total_leader_balance = sum([a.group.remaining_balance for a in leader_assignments]) or Decimal('0.00')

    final_amount_to_pay = total_amount_to_be_paid - Decimal(total_leader_balance)
...
def process_payment(request):
    ...
    try:
        ...
        print(f"Amount Input: {amount_paid}")
        ...
        print(f"Total Expenses (before advance): {total_expenses}")
        ...
        for assignment in leader_assignments:
            group = assignment.group
            used_from_group = min(group.remaining_balance, remaining_expenses)
            if used_from_group > 0:
                group.used_advance += used_from_group
                group.remaining_balance -= used_from_group  # deduct only the used part
                if group.remaining_balance <= 0:
                    group.remaining_balance = Decimal('0.00')
                    print("closing group after payment")
                    group.is_closed = True
                group.save()
                remaining_expenses -= used_from_group
                print(f"Advance group {group.name} updated → used_advance: {group.used_advance}, remaining_balance: {group.remaining_balance}")

        # --- Final amount to pay after advance ---
        final_amount_to_pay = max(remaining_expenses, Decimal('0.00'))
        print(f"Final Amount to Pay (after advance deduction): {final_amount_to_pay}")
...
def update_payment(request, transaction_id):
    # Filter by both user and transaction_id to ensure uniqueness
    payment = Payment.objects.filter(transaction_id=transaction_id, paid_to=request.user).order_by('-created_at').first()
    if not payment:
        messages.error(request, "Payment not found or you don’t have permission to edit it.")
    ...
    if request.method == "POST":
        ...
        if screenshot:
            ...
            messages.info(request, "No file selected for update.")
...
def directpay_preview(request):
    ...
    if request.method == "POST" and request.headers.get("x-requested-with") == "XMLHttpRequest":
        ...
        system_user = User.objects.get(id=16)
        created_count = 0
...
def delete_advance_log(request, log_id):
    ...
    if log.proof_file and os.path.exists(log.proof_file.path):
    ...
    # ------------------------------------------
    # 🔥 RECALCULATE TOTAL ADVANCE AFTER DELETE
    # ------------------------------------------

    # Get all remaining logs sorted by time
    logs = AdvanceGroupUpdateLog.objects.filter(group=group).order_by("timestamp")

    if logs.exists():
        # Final log has latest total
        latest_log = logs.last()
        group.total_advance = latest_log.new_amount
    else:
        # No logs → reset to group's initial value at creation
        # fallback: used total is 0 and group.total_advance unchanged
        pass

    # Recalculate used advance
    ...
    group.used_advance = used_total
    group.save()

    messages.success(request, "History record deleted and group balance updated.")

    return redirect(
        reverse("assign_advances") + f"?group_name={group.name}&action=history"
    )